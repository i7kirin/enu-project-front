  ...
  export default {
    ...
    methods: {
      ...
      async getInfo() {
        ...
        let response = await axios.get(`/api/statistics/it/employment/${this.professionTypeId}/${this.year}`)
        if (!response.error) {
          response.data.forEach(d => {
            this.chartOptions.xaxis.categories.push(d.name)
            this.series[0].data.push(d.employed);
            this.series[1].data.push(d.unemployed);
          })
        }
        this.chart = true;
      },
